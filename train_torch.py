...
class CharDataset(Dataset):
    def __init__(self, chats, tok_path, vocab, max_len=1500):
        self._data = chats
...
class KoGPT2Chat(LightningModule):
    ...
    def train_dataloader(self):
        data = pd.read_csv('Chatbot_data/help_massage_3.csv')
...
if __name__ == "__main__":
    if args.train:
        checkpoint_callback = ModelCheckpoint(
            filepath='model_chp/{epoch:02d}-{loss:.4f}',
            verbose=True,
            save_last=True,
            monitor='loss',
            mode='min',
